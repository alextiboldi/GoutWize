...
function buildTrustPoints(memberLabel: string) {
  return [
    { icon: Shield, text: "Anonymous by default" },
    { icon: Users, text: `${memberLabel} members` },
    { icon: Sparkles, text: "Free forever" },
  ];
}

export function LoginClient({ memberLabel = "Growing" }: { memberLabel?: string }) {
  const TRUST_POINTS = buildTrustPoints(memberLabel);
